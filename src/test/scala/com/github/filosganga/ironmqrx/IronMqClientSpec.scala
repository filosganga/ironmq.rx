  ...
  "IronMqClient" when {
    ...
    "deleteQueue is called" should {
      "delete the queue" in {
        ...
        ironMqClient.listQueues().futureValue should be(empty)
      }
    }
    ...
    "pushMessages is called" should {
      "return messages ids" in {
        val queue = givenQueue()
        ironMqClient.pushMessages(queue.name, PushMessage("test-1"), PushMessage("test-2")).futureValue.ids should have size 2
      }

      "push messages in the queue" in {
        val queue = givenQueue()
        ironMqClient.pushMessages(queue.name, PushMessage("test-1"), PushMessage("test-2")).futureValue.ids
        ironMqClient.pullMessages(queue.name, 20).futureValue.map(_.body).toSeq should contain inOrder("test-1", "test-2")
      }
    }
    ...
    "pullMessages is called" should {
      "remove messages from queue" in {
        val queue = givenQueue()
        ironMqClient.pushMessages(queue.name, PushMessage("test-1"), PushMessage("test-2")).futureValue.ids
        ironMqClient.pullMessages(queue.name, 2).futureValue.map(_.body).toSeq should contain inOrder("test-1", "test-2")
        ironMqClient.peekMessages(queue.name, 20).futureValue.toSeq should be(empty)
      }
    }

    "reserveMessages is called" should {
      "remove messages from queue temporarily" ignore {
        val queue = givenQueue()
        ironMqClient.pushMessages(queue.name, PushMessage("test-1"), PushMessage("test-2")).futureValue.ids
